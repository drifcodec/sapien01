        ...
        bcrypt.hash(default_password, 10, (err, hash) => {
          if (err) {
            ...
            const user = new User({
              ...
              user_id: req.body.name+"_"+(new Date()).getTime().toString(36),
              name: req.body.name,
            ...
            user.save()
              .then(result => {
                const token = jwt.sign({ _id: result._id, user_id: result.user_id, email: result.email }, "secret", { expiresIn: "1d" });
...
module.exports.forgot_password = (req, res) => {
  User.findOne({ user_id: req.params.user_id })
    ...
    .then(result => {
      if (result) {
        const token = jwt.sign({ _id: result._id, user_id: result.user_id, email: result.email }, "secret", { expiresIn: "1h" });
        reset_email.user_reset_password_email(result.email, token).catch(console.error);

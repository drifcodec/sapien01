const site_type = require('../../models/site_db/site_type')
const mongoose = require("mongoose");

module.exports.site_type_create = (req, res) => {
    var post_data = {
        _id: new mongoose.Types.ObjectId(),
        name: req.body.name,
        description: req.body.description,
    }
    const OrderObj_input = new site_type(post_data)
    OrderObj_input.save().then(data => {
        const message = {
            message: 'site_type created',
            status: 201,
...
module.exports.site_type_getList = (req, res) => {
...
module.exports.site_type_getList_table = (req, res) => {
    ...
    limit = req.body.length == undefined ? 1000 : req.body.length
    var recordsTotal = 0
    site_type.countDocuments({}, function(err, total) {
        recordsTotal = total
        site_type.countDocuments(drop_down_select, function(err, total_searched) {
            recordsFiltered = total_searched;

            //site.find(searchStr, '_id operator current_status') if i only want to return speficif fileds
            //site.find(searchStr) for globale search 
            site_type.find(drop_down_select)
                .skip(Number(start))
                .limit(Number(limit))
                .sort({
                    [order]: dir
...
module.exports.site_type_get = (req, res) => {
    const _id = req.params.id;
    site_type.findById(_id)
        .exec()
        .then(doc => {
...
module.exports.site_type_update = (req, res) => {
    const id = req.params.id;
    site_type.update({ _id: id }, { $set: req.body })
        .exec()
        .then(result => {
            if (result.nModified) {
                res.status(200).json({
...
module.exports.site_type_delete = (req, res) => {
    var id = req.params.id
    site_type.remove({ _id: id })
        .exec()
        .then(doc => {
            console.log("From database", doc);
            if (doc) {

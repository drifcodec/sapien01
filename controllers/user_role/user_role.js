...
module.exports.user_role_create = (req, res) => {
  ...
  const OrderObj_input = new user_role(post_data)
...
module.exports.user_role_getList = (req, res) => {
...
module.exports.user_role_getList_table = (req, res) => {
  ...
  limit = req.body.length == undefined ? 1000 : req.body.length
  var recordsTotal = 0
  user_role.countDocuments({}, function (err, total) {
    recordsTotal = total
    user_role.countDocuments(drop_down_select, function (err, total_searched) {
      recordsFiltered = total_searched;

      //user_role.find(searchStr, '_id operator current_status') if i only want to return speficif fileds
      //user_role.find(searchStr) for globale search 
      user_role.find(drop_down_select)
        .skip(Number(start))
        .limit(Number(limit))
        .sort({ [order]: dir })
        .exec()
...
module.exports.user_role_get = (req, res) => {
  const _id = req.params.id;
  user_role.findById(_id)
    .exec()
    .then(doc => {
...
module.exports.user_role_update = (req, res) => {
  const id = req.params.id;
  user_role.update({ _id: id }, { $set: req.body })
    .exec()
    .then(result => {
      if (result.nModified) {
        res.status(200).json({
...
module.exports.user_role_delete = (req, res) => {
  var id = req.params.id
  console.log("###########################_>>>>>>>>>"+id)
  user_role.remove({ _id: id })
    .exec()
    .then(doc => {
      console.log("From database", doc);
      if (doc) {

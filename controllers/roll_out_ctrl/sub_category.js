const sub_category = require('../../models/roll_out_db/sub_category')
const mongoose = require("mongoose");


module.exports.sub_category_create = (req, res) => {
    var post_data = {
    ...
    const OrderObj_input = new sub_category(post_data)
...
module.exports.sub_category_getList = (req, res) => {
...
module.exports.sub_category_getList_table = (req, res) => {
    ...
    limit = req.body.length == undefined ? 1000 : req.body.length
    var recordsTotal = 0
    sub_category.countDocuments({}, function(err, total) {
        recordsTotal = total
        sub_category.countDocuments(drop_down_select, function(err, total_searched) {
            recordsFiltered = total_searched;

            //site.find(searchStr, '_id operator current_status') if i only want to return speficif fileds
            //site.find(searchStr) for globale search 
            sub_category.find(drop_down_select)
                .skip(Number(start))
                .limit(Number(limit))
                .sort({
                    [order]: dir
...
module.exports.sub_category_get = (req, res) => {
    const _id = req.params.id;
    sub_category.findById(_id)
        .exec()
        .then(doc => {
...
module.exports.sub_category_update = (req, res) => {
    const id = req.params.id;
    sub_category.update({ _id: id }, { $set: req.body })
        .exec()
        .then(result => {
            if (result.nModified) {
                res.status(200).json({
...
module.exports.sub_category_delete = (req, res) => {
    var id = req.params.id
    console.log("###########################_>>>>>>>>>" + id)
    sub_category.remove({ _id: id })
        .exec()
        .then(doc => {
            console.log("From database", doc);
            if (doc) {
